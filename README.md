...
11. 스텝모터를 -90도 회전해 원래 위치로 복귀시킵니다.
12. 웜기어 모터를 다시 동작시킵니다.
13. 거리값이 60cm 기준보다 멀어질 때까지 대기합니다.
14. 거리값이 감지 기준보다 멀어지면 다시 감지 가능한 상태가 됩니다.
...
현재 `RESET_DIST`는 60cm입니다. 정확히 60cm 경계에서 반복 실행되지 않도록 코드는 60cm 기준보다 멀어진 값을 확인한 뒤 다음 감지를 허용합니다.
